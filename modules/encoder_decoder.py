...
from .att_model import pack_wrapper, AttModel, AttModel_single
...
class Transformer_edit(nn.Module):
    def __init__(self, decoder, src_embed, tgt_embed, rm):
        super(Transformer_edit, self).__init__()
        self.decoder = decoder
        self.src_embed = src_embed
        self.tgt_embed = tgt_embed
        self.rm = rm

    def forward(self, hidden_states, tgt, src_mask, tgt_mask):
        return self.decode(hidden_states, src_mask, tgt, tgt_mask)
...
class Encoder_edit(nn.Module):
    def __init__(self, layer, N, d_model, drop_prob_lm, d_vf):
        super(Encoder_edit, self).__init__()
        self.layers = clones(layer, N)
        self.norm = LayerNorm(layer.d_model)
        self.use_bn = 0
    ...
    def forward(self, fc_feats, att_feats):
        att_feats, att_masks = self._prepare_feature_forward(att_feats)
        memory = self.encode(att_feats, att_masks)
...
class EncoderDecoder(AttModel_single):
...
class Decoder_only(AttModel):
    ...
    def __init__(self, args, tokenizer):
        super(Decoder_only, self).__init__(args, tokenizer)
        self.args = args
        self.num_layers = args.num_layers
        self.d_model = args.d_model
        self.d_ff = args.d_ff
    ...
    def _prepare_feature(self, fc_feats, att_feats, memory, att_masks):
        return fc_feats[..., :1], att_feats[..., :1], memory, att_masks

    def _prepare_feature_forward(self, seq=None):
        if seq is not None:
            # crop the last one
            seq = seq[:, :-1]
            seq_mask = (seq.data > 0)
    ...
    def _forward(self, hiddenstate, att_masks, seq):
        seq, seq_mask = self._prepare_feature_forward(seq)
        out = self.model(hiddenstate, seq, att_masks, seq_mask)
        outputs = F.log_softmax(self.logit(out), dim=-1)
        return outputs
...
class Decoder_only_edit(AttModel):
    ...
    def __init__(self, args, tokenizer):
        super(Decoder_only_edit, self).__init__(args, tokenizer)
        self.args = args
        self.num_layers = args.num_layers
        self.d_model = args.d_model
        self.d_ff = args.d_ff
