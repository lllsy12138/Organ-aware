...
class AttModel(CaptionModel):
    ...
    def _sample_beam(self,
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(
            fc_feats, att_feats, memory, att_masks)

        assert beam_size <= self.vocab_size + 1, 'lets assume this for now, otherwise this corner case causes a few headaches down the road. can be dealt with in future if needed'
        seq = fc_feats.new_full((batch_size * sample_n, self.max_seq_length),
                                self.pad_idx,
                                dtype=torch.long)
    ...
    def _sample(self, fc_feats, att_feats, memory, att_masks=None):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, memory, att_masks,
                                     opt)
        if group_size > 1:
            return self._diverse_sample(fc_feats, att_feats, memory, att_masks,
                                        opt)
            ...
            if block_trigrams and t >= 3:
                ...
                for i in range(batch_size):  # = seq.size(0)
                    ...
                    if t == 3:  # initialize
                        trigrams.append(
                            {prev_two:
                             [current]})  # {LongTensor: list containing 1 int}
                    elif t > 3:
                ...
                mask = torch.zeros(
                    logprobs.size(),
                    requires_grad=False).cuda()  # batch_size x vocab_size
    ...
    def _diverse_sample(self,
        ...
        seq_table = [
            fc_feats.new_full((batch_size, self.max_seq_length),
                              self.pad_idx,
...
class AttModel_single(CaptionModel):
    def __init__(self, args, tokenizer):
        super(AttModel_single, self).__init__()
        self.args = args
        self.tokenizer = tokenizer
        self.vocab_size = len(tokenizer.idx2token)
        self.input_encoding_size = args.d_model
